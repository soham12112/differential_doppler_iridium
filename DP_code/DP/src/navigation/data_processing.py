...
def process_received_frames(frames_array: np.array, start_time: str, satellites: dict,
    ...
    # get actual satellite IDs, filtering out unmapped satellites
    tx_satellites_mapped = [(sat_id, map_sat_id_to_tle_id(sat_id)) for sat_id in tx_satellites]
    tx_satellites_mapped = [(iri_id, tle_id) for iri_id, tle_id in tx_satellites_mapped if tle_id is not False]
    tx_satellites_ids = [str(tle_id) for iri_id, tle_id in tx_satellites_mapped]
