...
from src.utils.plots import plot_results_of_iterative_position_finding, plot_analyzed_curve, \
    plot_measured_vs_trial_curve, plot_residuals_per_satellite, plot_measured_vs_predicted_doppler, \
    plot_measured_vs_predicted_with_true_location
...
def solve(nav_data, satellites, params: CurveFitMethodParameters, init_state: tuple[float, float, float, float, float] | None = None,
          true_location: tuple[float, float, float] | None = None):
    """
    ...
    # Plot residuals per satellite to identify ghost satellites
    if DEBUG.plot_residuals:
        plot_residuals_per_satellite(curve_array, lat, lon, alt, off, dft, satellites, show=False)
        plot_measured_vs_predicted_doppler(curve_array, lat, lon, alt, off, dft, satellites, show=False)
        
        # If true location provided, plot comparison
        if true_location is not None:
            lat_true, lon_true, alt_true = true_location
            plot_measured_vs_predicted_with_true_location(curve_array, lat, lon, alt, off, dft,
                                                         lat_true, lon_true, alt_true, satellites, show=False)
